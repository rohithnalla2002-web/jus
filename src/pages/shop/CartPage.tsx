...
export default function CartPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
      ...
      {lines.length === 0 ? (
        <div className="mt-12 rounded-2xl border border-dashed border-violet-300 bg-white/80 p-12 text-center text-zinc-600">
          <p>Your cart is empty.</p>
          <Link to="/products" className="mt-4 inline-block font-semibold text-violet-800 hover:underline">
            Browse products
          </Link>
        </div>
      ) : (
        <div className="mt-10 space-y-4">
          {lines.map((line) => {
            ...
            return (
              <motion.div
                key={line.productId}
                layout
                className="flex gap-4 rounded-2xl border border-violet-200/80 bg-white p-4 shadow-md"
              >
                <img src={getProductImageUrl(item)} alt={item.name} className="h-24 w-24 rounded-xl object-cover sm:h-28 sm:w-28" />
                <div className="flex flex-1 flex-col justify-between">
                  <div>
                    <Link to={`/product/${item.id}`} className="font-semibold text-zinc-900 hover:text-violet-800">
                      {item.name}
                    </Link>
                    <p className="text-sm text-violet-800">{item.price} each</p>
                  </div>
                  <div className="mt-3 flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="flex h-8 w-8 items-center justify-center rounded-lg border border-violet-200 text-zinc-800 hover:bg-violet-50"
                      >
                        âˆ’
                      </button>
          ...
          <div className="rounded-2xl border border-violet-200 bg-violet-50/80 p-6 shadow-inner">
            <div className="flex items-center justify-between text-lg">
              <span className="text-zinc-600">Total</span>
              <span className="font-serif text-2xl font-bold text-violet-900">{formatCurrency(subtotal)}</span>
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <button
                type="button"
                onClick={clearCart}
                className="rounded-xl border border-violet-300 px-5 py-3 text-sm font-semibold text-zinc-700 hover:bg-white"
              >
                Clear cart
              </button>
              <Link
                to="/products"
                className="rounded-xl bg-gradient-to-r from-violet-500 to-violet-700 px-6 py-3 text-sm font-bold text-white shadow-md"
              >
                Continue shopping
              </Link>
            </div>
